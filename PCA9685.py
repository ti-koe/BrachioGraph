...
import time
import math
# aliased to smbus in switch to Python3
...
class PCA9685:
  ...
  def _setPWM(self, channel, on, off):
    ...
    self._write(self.__LED0_ON_H+4*channel, on >> 8)
    self._write(self.__LED0_OFF_L+4*channel, off & 0xFF)
    self._write(self.__LED0_OFF_H+4*channel, off >> 8)
    if (self.debug):
  ...
  def setServoPulse(self, channel, pulse):
    ...
    self._setPWM(channel, 0, int(pulse))
